<template>
  <view class="delivery-info-card">
    ...
    <view class="delivery-status-section">
      <text class="status-text">{{ order.deliveryStatusText }}</text>
    </view>
    ...
    <view class="delivery-content-section">
      ...
      <view class="delivery-item" @click="$emit('make-phone-call')">
        ...
        <view class="item-content">
          ...
          <text class="item-value">{{ order.receiverName }} {{ order.receiverMobile }}</text>
        </view>
      ...
      <view class="delivery-item" @click="$emit('open-map')">
        ...
        <view class="item-content">
          ...
          <text class="item-value">{{ order.receiverDetailArea }} {{ order.receiverDetailAddress }}</text>
        </view>
        <view class="item-action">
...
<!--      <view class="delivery-item">
      ...
      <view class="delivery-item">
        ...
        <view class="item-content">
          ...
          <text class="item-value">{{ order.no }}</text>
        </view>
      </view>
      
      <!-- 订单备注 -->
      <view class="delivery-item" v-if="order.userRemark">
        <view class="item-icon">
          <uni-icons type="chat" size="20" color="#1890ff"></uni-icons>
        </view>
        <view class="item-content">
          <text class="item-label">订单备注</text>
          <text class="item-value">{{ order.userRemark }}</text>
        </view>
      </view>
    </view>
  </view>
